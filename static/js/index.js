...
window.app = Vue.createApp({
  ...
  methods: {
    ai-proxyFunction(data) {
      LNbits.api
        .request(
          'GET', // Type of request
          '/ai-proxy/api/v1/test/00000000', // URL of the endpoint
          this.g.user.wallets[0].inkey, // Often endpoints require a key
          data
        )
        .then(response => {
          this.ai-proxyData = response.data.map(mapObject) // Often whats returned is mapped onto some model
        })
    ...
    getVettedReadme() {
      ...
      LNbits.api
        .request('GET', '/ai-proxy/api/v1/vetted', this.g.user.wallets[0].inkey)
        .then(response => {
